...
// set theme automatically based on system theme on page reload
const isDark = window.matchMedia("(prefers-color-scheme:dark)").matches;
isDark ? darkMode.click() : "";
...
document.addEventListener("keydown", (eventKey) => {
	if (
		ansCharCount < 5 &&
		ansRowCount < 6 &&
		eventKey.code.substring(0, 3) === "Key"
	) {
		// fill answer box
		let popupShowList = [];
...
document.addEventListener("keydown", (eventKey) => {
	if (
		...
		warn.classList.contains("hidden") === false &&
		(eventKey.code === "Escape" ||
			eventKey.code === "Enter" ||
			eventKey.code === "NumpadEnter")
	) {
